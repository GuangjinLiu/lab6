...
void check_mouse(XEvent *e)
{
	...
	static int ct=0;
	static int min=0.0f;
	static int max=360.0f;
	static int right_agl = 90.0;
	static int ang = 0.05f;
		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					Flt rad = ((g.ship.angle+right_agl) / max) * PI * 2.0;
